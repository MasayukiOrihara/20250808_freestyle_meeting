...
server.addTool({
    name: "search_web",
    description: "検索ワードを受け取り、検索結果を文字列で返す関数",
    parameters: zod_1.z.object({
        word: zod_1.z.string(),
    }),
    execute: (_a) => __awaiter(void 0, [_a], void 0, function* ({ word }) {
        return yield searchWebWithTavily(word);
    }),
});
server.addTool({
    name: "search_weather",
    description: "日付と地域を受け取り、天気予報の検索結果を文字列で返す関数。地域の指定がない場合は「東京」を指定の地域として返します。",
    parameters: zod_1.z.object({
        area: zod_1.z.string(),
        date: zod_1.z.string(),
    }),
    execute: (_a) => __awaiter(void 0, [_a], void 0, function* ({ area, date }) {
        const query = "site:tenki.jp/ " + area + " " + date;
        return yield searchWebWithTavily(query);
    }),
});
server.addTool({
    name: "get_today",
    description: "今日の日付と時間を文字列で返す関数。",
